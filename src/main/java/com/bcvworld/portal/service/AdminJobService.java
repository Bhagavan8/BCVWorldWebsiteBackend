...
public class AdminJobService {
	...
	public Page<JobResponse> getJobsForManagement(String search, Pageable pageable) {
		...
		Page<Long> jobIdsPage = jobRepository.findJobIdsForManagement(search, pageable);
		...
		Map<Long, Job> jobMap = jobRepository.findJobsWithDetails(ids).stream()
				.collect(Collectors.toMap(Job::getId, Function.identity()));

		List<JobResponse> responses = ids.stream().map(jobMap::get).filter(Objects::nonNull).map(this::mapToJobResponse)
				.toList();

		return new PageImpl<>(responses, pageable, jobIdsPage.getTotalElements());
	}
	...
	public Job updateJob(Long id, Job request) {
		...
		// Posted date
		if (existing.getPostedDate() == null) {
